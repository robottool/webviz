...
The repo is a **vertical slice in progress**. Working tabs: **Inspector** and **3D** (with `RobotModel`, `TFFrames`, `Marker`, and `PointCloud` display plugins, TFManager, and SceneManager). The tab store models all six types (`3d`, `image`, `plot`, `map`, `inspector`, `log`); Image/Plot/Map/Log renderers, the rest of the plugin catalogue (LaserScan, OccupancyGrid, Path, Pose, …), recording, and C++/ROS2 SDKs are not yet built.
