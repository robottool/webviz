...
The repo is a **vertical slice in progress**. Working tabs: **Inspector** and **3D** (with `RobotModel`, `TFFrames`, and `Marker` display plugins, TFManager, and SceneManager). The tab store models all six types (`3d`, `image`, `plot`, `map`, `inspector`, `log`); Image/Plot/Map/Log renderers, the rest of the plugin catalogue (PointCloud, LaserScan, OccupancyGrid, …), recording, and C++/ROS2 SDKs are not yet built.
