...
- `tabs/TabRenderer.tsx` dispatches a `TabConfig` to its renderer; only `InspectorTab.tsx` is live, others fall back to `PlaceholderTab.tsx`.
- **3D tab** (`tabs/ThreeDTab.tsx`) — three-column workspace (Displays sidebar · Three.js viewport · schema-driven Properties form). Per-tab `SceneManager`; the TF tree and hub connection are shared singletons.
...
- `plugins/RobotModelPlugin.ts` — loads URDF via `urdf-loader`, poses the robot from TF (`resolveToFixed`) each render, animates joints from `wv/JointState`. Mesh paths are rewritten onto the hub asset server by anchoring on `ur_description/` (the URDF's declared package name is ignored), with per-extension loaders (Collada/STL/GLTF).
